...
LPWSTR GetRegStr_StrTblDefault(
    UINT uPathID,
    UINT uKeyID,
    UINT uDefaultID,
    BOOL bStore
)
{
    ...
    LoadString( _hInst, uPathID, szPath, sizeof(szPath) / sizeof(szPath[0]) );
    LoadString( _hInst, uKeyID, szKey, sizeof(szKey) / sizeof(szPath[0]) );
    LoadString( _hInst, uDefaultID, szDefault, sizeof(szDefault) / sizeof(szPath[0]) );
...
)
{
    ...
    LPVOID lpvValue = NULL; // Initialize for error path
    ...
    if( !bFound )
    {
        // Fall through error path so we close hkey, thanks Prefix
        if( szDefault == NULL )
    		goto lExit;
        
        // use default
        dwSzLen = lstrlen(szDefault) * sizeof(szDefault[0]); // size in bytes
    ...
    if (bFound)
    {
        lstrcpyn((WCHAR *)lpvValue, szTmp, dwSzLen+1); //allocated dwSzLen+1 bytes, where length of szTemp <= dwSzLen 
    }
    else 
    {
        ExitOnNull(szDefault);
        lstrcpyn((WCHAR *)lpvValue, szDefault, dwSzLen+1); //allocated dwSzLen+1 bytes, where length of szDefault <= dwSzLen 
    }
...
HICON GetShellIcon( LPCWSTR pszItemName, BOOL bDirectory )
{
    ...
    if (iRetVal)
    {
        if( bDirectory )
        {
            ...
            wcscpy( szTempFile, szTempPath ); //copying into same length string, hence OK
        }
        else 
        {
