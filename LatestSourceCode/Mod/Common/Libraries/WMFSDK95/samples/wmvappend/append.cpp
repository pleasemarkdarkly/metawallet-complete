                                 ...
                                 /* [in] */ void __RPC_FAR * pvContext )
{
    ...
    {
        _tcscat( tszFlags, _T(" WM_SF_CLEANPOINT ") );
    }
    if( dwFlags & WM_SF_DISCONTINUITY )
    {
        _tcscat( tszFlags, _T(" WM_SF_DISCONTINUITY ") );
    }
    if( dwFlags & WM_SF_DATALOSS )
    {
        _tcscat( tszFlags, _T(" WM_SF_DATALOSS ") );
    }
...
//       media types.
//------------------------------------------------------------------------------
HRESULT CAppend::CompareProfiles(WCHAR* pwszFirstInfile, WCHAR* pwszSecondInfile, BOOL* pIsEqual)
{
...
//------------------------------------------------------------------------------
HRESULT CAppend::Configure( WCHAR * pwszOutFile )
{
    ...
    do
    {
        ...
        for( UINT i = 0; i < m_dwStreamCount; i++ )
        {
			//
        ...
        wcscpy( m_pwszOutFile, pwszOutFile );
...
HRESULT CAppend::CopyCodecInfo( IWMHeaderInfo * pWHdrInfo )
{
    ...
    WCHAR *     pwszName = NULL, * pwszDescription = NULL;
...
//------------------------------------------------------------------------------
HRESULT CAppend::IsFileProtected( WCHAR * pswzFileName )
{
    IWMMetadataEditor *     pEditor     = NULL;
