...
public:
	...
	HRESULT Configure( WCHAR * pwszOutFile );
	HRESULT CompareProfiles( WCHAR * pwszFirstInfile, WCHAR* pwszSecondInfile, BOOL * pIsEqual );
	HRESULT Exit();
	HRESULT Init();
	
private:

    HRESULT IsFileProtected( WCHAR* pswzFileName);
    HRESULT CopyAllMarkers();
    HRESULT CopyAttribute( WORD nInStreamNum,  WORD nOutStreamNum, IWMHeaderInfo * pWriterHeaderInfo, const WCHAR * pwszName );
