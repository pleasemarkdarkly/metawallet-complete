...
HRESULT IndexFileByFrames(WCHAR *wszTargetFile);
...
//------------------------------------------------------------------------------
HRESULT CopyASF(int argc, TCHAR *argv[])
{
    HRESULT hr;    
    WCHAR wszSourceFile[MAX_PATH], wszTargetFile[MAX_PATH];
    DWORD dwRequestedProfile=0, dwRequestedVersion = (DWORD) g_WMTVersion;
    int i = 1;
...
#else
    wcsncpy(wszTargetFile, argv[argc-1], NUMELMS(wszTargetFile));
#endif
...
#else
        wcsncpy(wszSourceFile, argv[i], NUMELMS(wszSourceFile));
#endif
    ...
    if(SUCCEEDED(hr))
    {
        ...
        if (fMultipassEncode)
        {
            ...
            if (pMS)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    nEvent = WaitForCompletion(pGraph);           
