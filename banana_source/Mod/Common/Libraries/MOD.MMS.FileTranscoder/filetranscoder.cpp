...
WCHAR CFileTranscoder::s_szFile[TRANSCODER_ERROR_LOG_STRING_LEN];
WCHAR CFileTranscoder::s_szFunction[TRANSCODER_ERROR_LOG_STRING_LEN];
int CFileTranscoder::s_line;
HRESULT CFileTranscoder::s_hr;

void
CFileTranscoder::LogErrorLocation(HRESULT hr, const char* szFile, const char* szFunction, int line)
{
//    printf("Error: hr = 0x%08x, in file %s, function %s, line %d\n", hr, szFile, szFunction, line);
    s_hr = hr;
    MultiByteToWideChar(CP_ACP, 0, szFile, TRANSCODER_ERROR_LOG_STRING_LEN, s_szFile, TRANSCODER_ERROR_LOG_STRING_LEN);
    s_szFile[TRANSCODER_ERROR_LOG_STRING_LEN - 1] = '\0';
    MultiByteToWideChar(CP_ACP, 0, szFunction, TRANSCODER_ERROR_LOG_STRING_LEN, s_szFunction, TRANSCODER_ERROR_LOG_STRING_LEN);
    s_szFunction[TRANSCODER_ERROR_LOG_STRING_LEN - 1] = '\0';
    s_line = line;
}

void
CFileTranscoder::GetErrorLocation(int* pHR, LPWSTR szFile, DWORD cchFile, LPWSTR szFunction, DWORD cchFunction, int* pLine)
{
    if (pHR != 0)
    {
        *pHR = s_hr;
...
//------------------------------------------------------------------
/// <summary>File Transcoder constructor.</summary>
// ------------------------------------------------------------------
CFileTranscoder::CFileTranscoder()
{
    m_pProfile = NULL;
...
//------------------------------------------------------------------
/// <summary>File Transcoder destructor.</summary>
// ------------------------------------------------------------------
CFileTranscoder::~CFileTranscoder()
{
...
// ------------------------------------------------------------------
HRESULT
CFileTranscoder::InitializeDefaultProfile()
{
    HRESULT hr;
...
//    CHECK_HR(AddStreamToProfile(1, 16, 44100, 128 * 1024));
    CHECK_HR(AddStreamToProfile(2, 16, 44100, 128 * 1024));
...
// ------------------------------------------------------------------
HRESULT
CFileTranscoder::InitializeProfile()
{
    HRESULT hr;
...
//------------------------------------------------------------------
/// <summary>Transcodes the given input file.</summary>
/// 
/// <returns>An HRESULT indicating success or failure</returns>
// ------------------------------------------------------------------
HRESULT
CFileTranscoder::TranscodeFile(
...
//------------------------------------------------------------------------------
// Name: CFileTranscoder::GetOutputMap()
// Desc: Creates the map from the reader ouputs to the writer inputs and 
//       the map from the reader outputs to the reader streams.
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::GetOutputMap()
{
    HRESULT             hr = S_OK;
...
//------------------------------------------------------------------------------
// Name: CFileTranscoder::SetWriterInput()
...
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::SetWriterInput( BOOL fMultiChannel, 
...
//------------------------------------------------------------------------------
// Name: CFileTranscoder::Preprocess()
// Desc: Preprocesses the samples from the reader.
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::Preprocess()
{
    HRESULT     hr = S_OK;
    ...
    while( TRUE )
    {
        ...
        // Stop the reader
        //
        // ED: This causes the OnStatus call to randomly receive 0x80004004 errors
        // ED: Commenting out the calls doesn't affect the transcoding
//        hr = m_pReader->Stop( );
        PRINT_DEBUG( _T( "\n" ) );
        if( FAILED( hr ) )
        {
...
//------------------------------------------------------------------------------
// Name: CFileTranscoder::Process()
// Desc: Processes the samples from the reader.
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::Process()
{
    HRESULT hr = S_OK;
...
// ------------------------------------------------------------------
HRESULT
CFileTranscoder::CreateProfile(IWMProfile ** ppIWMProfile)
{
    HRESULT             hr = S_OK;
...
// ------------------------------------------------------------------
HRESULT
CFileTranscoder::AddStreamToProfile(
    WORD wChannels,
    WORD wBitsPerSample,
...
HRESULT CFileTranscoder::FindAudioFormat(WAVEFORMATEX* pWaveLimits,
                        DWORD dwMaxRate,
                        BOOL fUseVBR,
                        DWORD dwNumPasses,
                        IWMStreamConfig** ppStreamConfig)
{
    ...
    DWORD dwFormatIndex = 0;
    DWORD cCodecEntries = 0;
    DWORD cFormatEntries = 0;
    DWORD dwBestRate = 0;
    DWORD PacketsPerSecond = 0;
    DWORD dwCodecIndex = 0; 
    ...
    hr = pCodecInfo->GetCodecInfoCount(WMMEDIATYPE_Audio, &cCodecEntries);
    GOTO_EXIT_IF_FAILED(hr);

    // Find the dwFormatIndex of the codec corresponding to the requested subytpe.
    for (dwCodecIndex = 0; dwCodecIndex < cCodecEntries; dwCodecIndex++)
    {
        hr = pCodecInfo->SetCodecEnumerationSetting(WMMEDIATYPE_Audio, dwCodecIndex, g_wszVBREnabled, WMT_TYPE_BOOL, (const BYTE*)&fUseVBR, sizeof(fUseVBR));
        if (FAILED(hr))
        {
            continue;
        }
        // E_UNSUPPORTED_PROPERTY = 0xC00D0BCD

        hr = pCodecInfo->SetCodecEnumerationSetting(WMMEDIATYPE_Audio, dwCodecIndex, g_wszNumPasses, WMT_TYPE_DWORD, (const BYTE*)&dwNumPasses, sizeof(dwNumPasses));
        if (FAILED(hr))
        {
            continue;
        }
        ...
        for(dwFormatIndex = 0; dwFormatIndex < cFormatEntries; dwFormatIndex++)
        {
            // Get the next format.
            ...
            // Start checking data.
            if (HIWORD(pWave->nAvgBytesPerSec) == 0x7FFFFF)
            {
                hr = hr;
            }
...
// ------------------------------------------------------------------
HRESULT
CFileTranscoder::CreateReader(const WCHAR * wszOutputFile)
{
    HRESULT hr = S_OK;
...
//------------------------------------------------------------------------------
// Name: CFileTranscoder::CreateWriter()
// Desc: Creates a writer and sets the profile and output of this writer.
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::CreateWriter( const WCHAR * wszOutputFile, 
                                      IWMProfile * pProfile )
{
    HRESULT             hr = S_OK;
...
//------------------------------------------------------------------------------
// Name: CFileTranscoder::OnSample()
// Desc: Implementation of IWMReaderCallback::OnSample.
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::OnSample( /* [in] */ DWORD dwOutputNum,
...
/*
            if (((DWORD)(qwSampleTime / 10000000)) >= m_dwSampleDurationSeconds)
            {
                SetEvent(m_hEvent);
            }
*/
        ...
        if( FAILED( hr ) )
        {
            _tprintf( _T( "OnSample error: 0x%08x\n" ), hr );
            m_hr = hr;
            SetEvent( m_hEvent );
        }
    }
...
//------------------------------------------------------------------------------
// Name: CFileTranscoder::OnStatus()
// Desc: Implementation of IWMStatusCallback::OnStatus.
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::OnStatus( /* [in] */ WMT_STATUS Status,
                                  /* [in] */ HRESULT hr,
                                  ...
                                  /* [in] */ void __RPC_FAR * pvContext)
{
    ...
    if( FAILED(hr) )
    {
        _tprintf( _T( "OnStatus 1 status: %d, hr: 0x%08x\n" ), (int)Status, hr );
        m_hr = hr;
        SetEvent( m_hEvent );
        return(S_OK);
    }
    ...
    case WMT_STARTED:
        ...
        if( FAILED( hr ) )
        {
            _tprintf( _T( "OnStatus 2 hr: 0x%08x\n" ), hr );
            m_hr = hr;
            SetEvent( m_hEvent );
        }
...
//------------------------------------------------------------------------------
// Name: CFileTranscoder::OnTime()
// Desc: Implementation of IWMReaderCallbackAdvanced::OnTime.
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::OnTime( /* [in] */ QWORD qwCurrentTime,
                                /* [in] */ void __RPC_FAR * pvContext)
{
    //
    ...
    if( !m_fEOF )
    {
		...
		else if (m_pReaderAdvanced != NULL)
        {
            ...
            if( FAILED( hr ) )
            {
                ...
                // If an error occurred in the reader, save this error code and set the event.
                //
                _tprintf( _T( "OnTime 2 hr: 0x%08x\n" ), hr );
                m_hr = hr;
                SetEvent( m_hEvent );
            }
        }
...
//------------------------------------------------------------------------------
// Name: CFileTranscoder::OnStreamSample()
// Desc: Implementation of IWMReaderCallbackAdvanced::OnStreamSample.
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::OnStreamSample( /* [in] */ WORD wStreamNum,
...
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::OnStreamSelection( /* [in] */ WORD wStreamCount,
...
HRESULT CFileTranscoder::OnOutputPropsChanged( /* [in] */ DWORD dwOutputNum,
...
HRESULT CFileTranscoder::AllocateForOutput( /* [in] */ DWORD dwOutputNum,
                                           /* [in] */ DWORD cbBuffer,
...
HRESULT CFileTranscoder::AllocateForStream( /* [in] */ WORD wStreamNum,
                                           /* [in] */ DWORD cbBuffer,
                                           /* [out] */ INSSBuffer __RPC_FAR *__RPC_FAR * ppBuffer,
                                           /* [in] */ void __RPC_FAR * pvContext)
{
...
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::QueryInterface( /* [in] */ REFIID riid,
...
ULONG CFileTranscoder::AddRef()
{
    return( InterlockedIncrement( &m_cRef ) );
}

ULONG CFileTranscoder::Release()
{
...
//------------------------------------------------------------------------------
// Name: CFileTranscoder::WaitForCompletion()
// Desc: Waits until the event is signaled.
//------------------------------------------------------------------------------
HRESULT CFileTranscoder::WaitForCompletion()
{
    HRESULT hr = S_OK;
    ...
    if( WAIT_OBJECT_0 == dwResult )
    {
        ...
        if (FAILED(hr))
        {
            _tprintf( _T( "WaitForSingleObject, m_hr = 0x%08x\n" ), hr );
        }
    }
    else
    {
        _tprintf( _T( "WaitForSingleObject, returned 0x%08x\n" ), dwResult );
