...
HRESULT CAudioPlay::Init()
{
	...
	do
	{
		...
		if( FAILED( hr ) )
		{
			_tcscpy( tszErrMsg, _T( "CoInitialize failed" ) );
			break;
		}
		...
		{
			_tcscpy( tszErrMsg, _T( "Could not create the event" ) );
			hr = E_FAIL;
			break;
		}
		...
		if( FAILED( hr ) )
		{
			_tcscpy( tszErrMsg, _T( "Could not create Reader" ) );
			break;
		}
	}
	while( FALSE );
	...
	if( FAILED( hr ) )
	{
        wsprintf( tszErrMsg, _T("%s (hr=%#X)"), tszErrMsg, hr );
...
HRESULT CAudioPlay::Open( LPCWSTR pwszUrl )
{
	...
	do
	{
		...
		if( NULL == m_pwszURL )
		{
			...
			_tcscpy( tszErrMsg, _T( "Insufficient memory" ) );
			break;
		}
		...
		if( FAILED( hr ) )
        {
            _tcscpy( tszErrMsg, _T( "Could not open the specified file" ) );
			break;
		}
		...
		if( FAILED (hr) )
		{
			_tcscpy( tszErrMsg, _T( "Could not QI for IWMHeaderInfo" ) );
			break;
		}
...
HRESULT CAudioPlay::GetAudioOutput()
{
	...
	do
	{
		...
		if( FAILED( hr ) )
		{
			_tcscpy( tszErrMsg, _T( "Could not get output count" ) );
			break;
		}
		...
		for( i = 0; i < cOutputs; i++ )
		{
			...
			if( FAILED( hr ) )
			{
				_tcscpy( tszErrMsg, _T( "Could not get output props" ) );
				break;
			}
			...
			if( FAILED( hr ) )
			{
				_tcscpy( tszErrMsg, _T( "Could not get media type" ) );
				break;
			}
		...
		if( i == cOutputs )
		{
			...
			_tcscpy( tszErrMsg, _T( "Could not find an audio stream in the specified file" ) );
			break;
		}
...
HRESULT CAudioPlay::RetrieveAndDisplayAttributes()
{
	...
	do
	{
		...
		if( FAILED( hr ) )
		{
			_tcscpy( tszErrMsg, _T( "Could not get the Title attribute" ) );
			break;
		}
		...
		if( FAILED( hr ) )
		{
			_tcscpy( tszErrMsg, _T( "Could not get the Author attribute" ) );
			break;
		}
		...
		if( FAILED( hr ) )
		{
			_tcscpy( tszErrMsg, _T( "Could not get the Copyright attribute" ) );
			break;
		}
		...
		if( FAILED( hr ) )
		{
			_tcscpy( tszErrMsg, _T( "Could not get the Duration attribute" ) );
			break;
		}
		...
		if( FAILED( hr ) )
		{
			_tcscpy( tszErrMsg, _T( "Could not get the Seekable attribute" ) );
			break;
		}
		...
		if( FAILED( hr ) )
		{
			_tcscpy( tszErrMsg, _T( "Could not get the Broadcast attribute" ) );
			break;
		}
