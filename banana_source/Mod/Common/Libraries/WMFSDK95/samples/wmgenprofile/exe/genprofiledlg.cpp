...
void CGenProfileDlg::OnSelchangeCBStreamType() 
{
    ...
    do
    {
        ...
        nSelectedType = m_cbStreamType.GetItemData( nSelectedIndex );
...
void CGenProfileDlg::OnSelchangeCBStreamCodec() 
{
    ...
    case ST_Audio:
        ...
        dwCodecIndex = m_cbStreamCodec.GetItemData( nSelectedIndex );
        ...
        dwStreamFormatIndex = m_cbStreamFormat.GetItemData( dwStreamFormatStringIndex );
...
void CGenProfileDlg::OnSelchangeCBStreamFormat() 
{
    ...
    if ( !((CStream*) m_pDisplayedProfileObject)->GetIsUncompressed() )
    {
        ...
        dwFormatIndex = m_cbStreamFormat.GetItemData( nSelectedIndex );
    ...
    else
    {
        ...
        DWORD dwIndex = m_cbStreamFormat.GetItemData( dwStringIndex );
...
void CGenProfileDlg::OnSelchangeCBStreamVideoVBRMode() 
{
    ...
    do
    {
        ...
        dwCodecIndex = m_cbStreamCodec.GetItemData( dwSelectedCodec );
...
HRESULT CGenProfileDlg::ShowAudioStream( CStream *pStream )
{
    ...
    do
    {
        ...
        if ( !fIsUncompressedStream )
        {
            ...
            //
            dwCodecIndex = m_cbStreamCodec.GetItemData( nCodecStringIndex );
            assert( CB_ERR != dwCodecIndex );
            if ( CB_ERR == dwCodecIndex )
            {
                dwCodecIndex = 0;
        ...
        else
        {
            ...
            pStream->SetWaveFormatIndex( m_cbStreamFormat.GetItemData( dwWaveFormatIndex ) );
        }
...
HRESULT CGenProfileDlg::PopulatePixleFormatCB()
{
    ...
    for ( DWORD i = 0; i < dwFormatCount; i ++ )
    {
        ...
        if ( wBitsPerPixel > 8 )
        {
            ...
            if ( dwFourCC == BI_RGB )
            {
                _stprintf( tszName, _T("RGB%d"), (DWORD)wBitsPerPixel );
            }
...
HRESULT CGenProfileDlg::AddAudioFormatsToCB( BOOL fIsVBR, DWORD dwNumVBRPasses, DWORD dwCodecIndex )
{
    ...
    do
    {
        ...
        for ( dwFormatIndex = 0; dwFormatIndex < dwFormatCount; dwFormatIndex++ )
        {
            ...
            wszFormatName = new WCHAR[ dwFormatNameLength + 1 ];
            if ( !wszFormatName )
            {
                hr = E_OUTOFMEMORY;
                break;
            ...
            wszFormatName[ dwFormatNameLength ] = L'\0'; // Terminate the string, just in case
...
HRESULT CGenProfileDlg::ShowVideoStream( CStream *pStream )
{
    ...
    do
    {
        ...
        else
        {
            ...
            pStream->SetPixelFormatIndex( m_cbPixelFormat.GetItemData( dwPixelFormatStringIndex ) );
        }
...
#ifdef UNICODE
        wcscpy( wszProfileName, strProfileName );
#else // UNICODE
...
#ifdef UNICODE
        wcscpy( wszProfileDescription, (LPCTSTR) strProfileDescription );
#else // UNICODE
...
void CGenProfileDlg::OnSelchangeCBPixelFormat() 
{
    ...
    DWORD dwIndex = m_cbPixelFormat.GetItemData( dwStringIndex );
...
void CGenProfileDlg::OnSelchangeCBLanguage() 
{
    ...
    LCID lcid = m_cbLanguage.GetItemData( dwStringIndex );
...
HRESULT CGenProfileDlg::PopulateLanguageCB()
{
    ...
    do
    {
        ...
        while( SUCCEEDED( hr ) )
        {
...
#ifdef UNICODE
            _sntprintf( szLanguageName, 256, _T("%s [%s]"), rfcinfo.wszLocaleName, rfcinfo.wszRfc1766 );

            // prefast fix #140662 - _sntprintf may not null terminate result string
            szLanguageName[ sizeof( szLanguageName )/sizeof( WCHAR )  - 1 ] = L'\0';
            ...
            _sntprintf( szLanguageName, 256, _T("%s [%s]"), szLocaleName, szRfc1766 );
