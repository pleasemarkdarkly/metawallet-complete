...
//------------------------------------------------------------------------------
HRESULT ConvertCharToTChar( CHAR * pszInput, TCHAR ** ptszOutput )
{
    int cchOutput = 0;
...
#else //UNICODE
    strncpy( *ptszOutput, pszInput, cchOutput );
#endif // UNICODE
...
//------------------------------------------------------------------------------
HRESULT ConvertTCharToChar( TCHAR * ptszInput, CHAR ** pszOutput )
{
    int cchOutput = 0;
...
#else //UNICODE
    strncpy( *pszOutput, ptszInput, cchOutput );
#endif // UNICODE
...
// parameter settings.
//------------------------------------------------------------------------------
HRESULT CUncompAVIToWMV::Initial( TCHAR*         ptszInFile,
...
//------------------------------------------------------------------------------
HRESULT CUncompAVIToWMV::InitAVISource( TCHAR* ptszInputFile )
{
    HRESULT         hr = S_OK;
                                            ...
                                            IWMProfile ** ppIWMProfile )
{
    ...
    if( NULL == ptszProfileFile || NULL == ptszProfileFile )
    {
        return( E_POINTER );
    }
...
HRESULT CUncompAVIToWMV::DoPreprocessing()
{
    ...
    while( SUCCEEDED( hr ) && NULL != pStream )
    {
            ...
            WMMEDIATYPE_Audio == pStream->m_Type )
        {
            ...
            dwCurrentSample = pStream->m_dwCurrentSample;
